...
export function ParentBehaviorView({ user, childId }: any) {
  ...
  useEffect(() => {
    const loadBehavior = async () => {
      try {
        ...
        const response = await fetch(`/api/behavior?studentId=${childId}`, {
      ...
      <div className="space-y-2">
        {behavior.map(record => (
          <div key={record.id} className="border rounded-lg p-3">
            ...
            <p className="text-xs text-gray-600 mt-1">Rögzítette: {record.recordedByName}</p>
          </div>
        ))}
      </div>
    </div>
